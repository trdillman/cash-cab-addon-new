"""
Script Generator Fix
"""

from pathlib import Path

def main():
    """Generate test scripts with proper paths"""
    current_dir = Path(__file__).parent
    output_dir = current_dir / "scripts"
    ...
    # Generate a simple test script for each case
...
Expected Result: {test_config["expected_result"]}
Parent Clearing: {test_config["parent_clearing"]}
"""

def log_debug(message: str, level: str = "INFO"):
    print(f"[{test_name.upper()}] [{{level}}] {{message}}")
...
def main():
    ...
    try:
        ...
        # Apply workflow
        apply_workflow(objects, {test_config["parent_clearing"]})
...
'''
    ...
    print(f"\\nSuccessfully generated {len(test_cases)} test scripts")
